...
class Terrain:
    ...
    def _generate_terrain(self, path):
        ...
        # 2. Generowanie siatki wysokości z organicznym szumem (dwie oktawy)
        heights = []
        for z in range(height):
            ...
            for x in range(width):
                ...
                if base_y > 0.1:
                    # Łączymy dwa nakładające się szumy o różnych częstotliwościach i obrotach
        ...
        # 4. Generowanie wierzchołków ze zmiękczonym cieniowaniem
        vertices = []
        half_size = self.size / 2.0

        def calculate_shaded_color(y, normal):
            slope = normal[1] # 1.0 = płasko
            t = (y - self.min_height) / (self.max_height - self.min_height)
            t = max(0.0, min(1.0, t))

            # Chłodne, deszczowe palety barw
            sand = (0.32, 0.28, 0.22)
            ...
            # Miękkie światło rozproszone z nieba (Ambient + Diffuse)
            light_dir = (0.2, 0.8, 0.3)
            dot_val = normal[0] * light_dir[0] + normal[1] * light_dir[1] + normal[2] * light_dir[2]
            
            # Zmiękczenie kontrastu cieni (clamp od 0.45 zamiast ostrych przejść)
